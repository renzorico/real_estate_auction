...
def fetch_property_data(property_data_list):
    data_list = []

    for property_id_full in property_data_list:
        property_id = property_id_full.split(" ")[1]

        property_url = (
            f"https://subastas.boe.es/detalleSubasta.php?idSub={property_id}&ver=1"
        )
        ...
        if property_info.startswith("Subasta SUB-"):
            ...
            if div_id_bloque_datos3:
                ...
                for data_div in data_divs:
                    property_data = {
                        "Descripción": property_info,
                        "Identificador": property_id,
                    }
                    ...
                    if property_table:
                        ...
                        for row in rows:
                            ...
                            td = row.find("td")
                            if th and td:
                                property_data[th.get_text(strip=True)] = td.get_text(
                                    strip=True
                                )
